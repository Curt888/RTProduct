package com.example.rtproduct.manager
...
class MQTTmanager (val connectionParams: MQTTConnectionParams, val context: Context, val uiUpdater: UIUpdaterInterface?) {
    ...
    var client = MqttAndroidClient(context,connectionParams.host,connectionParams.clientId + id(context))
...
//                    Log.w("topic", "Topic Subscribe")
//                    uiUpdater?.updateStatusViewWith("Connected")
                    uiUpdater?.updateStatusViewWith("Subscribed to Topic")

                }
                override fun onFailure(asyncActionToken:IMqttToken, exception:Throwable) {
                    Log.w("Mqtt", "Subscription fail!")
//                    uiUpdater?.updateStatusViewWith("Failed")
...
data class MQTTConnectionParams(val clientId:String, val host: String, val topics: Array<String>, val username: String, val password: String) {
}